	...
	, mLastClick(-1.0f)
{
	...
	SetLanguageDefinition(LanguageDefinition::NGC());
...
#define GetKeyIndex(a) a

		if (!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_Z)))
			Undo();
		else if (!IsReadOnly() && !ctrl && !shift && alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_Backspace)))
			Undo();
		else if (!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_Y)))
			Redo();
		else if (!ctrl && !alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_UpArrow)))
			MoveUp(1, shift);
		else if (!ctrl && !alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_DownArrow)))
			MoveDown(1, shift);
		else if (!alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_LeftArrow)))
			MoveLeft(1, shift, ctrl);
		else if (!alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_RightArrow)))
			MoveRight(1, shift, ctrl);
		else if (!alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_PageUp)))
			MoveUp(GetPageSize() - 4, shift);
		else if (!alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_PageDown)))
			MoveDown(GetPageSize() - 4, shift);
		else if (!alt && ctrl && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_Home)))
			MoveTop(shift);
		else if (ctrl && !alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_End)))
			MoveBottom(shift);
		else if (!ctrl && !alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_Home)))
			MoveHome(shift);
		else if (!ctrl && !alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_End)))
			MoveEnd(shift);
		else if (!IsReadOnly() && !ctrl && !shift && !alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_Delete)))
			Delete();
		else if (!IsReadOnly() && !ctrl && !shift && !alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_Backspace)))
			BackSpace();
		else if (!ctrl && !shift && !alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_Insert)))
			mOverwrite ^= true;
		else if (ctrl && !shift && !alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_Insert)))
			Copy();
		else if (ctrl && !shift && !alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_C)))
			Copy();
		else if (!IsReadOnly() && !ctrl && shift && !alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_Insert)))
			Paste();
		else if (!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_V)))
			Paste();
		else if (ctrl && !shift && !alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_X)))
			Cut();
		else if (!ctrl && shift && !alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_Delete)))
			Cut();
		else if (ctrl && !shift && !alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_A)))
			SelectAll();
		else if (!IsReadOnly() && !ctrl && !shift && !alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_Enter)))
			EnterCharacter('\n', false);
		else if (!IsReadOnly() && !ctrl && !alt && ImGui::IsKeyPressed(GetKeyIndex(ImGuiKey_Tab)))
...
///			snprintf(buf, 16, "%d  ", lineNo + 1);
			...
			if (mState.mCursorPosition.mLine == lineNo) {
				...
				if (false && !HasSelection()) {	// DY:disabled
...
if (g.InputTextState.ID != g.CurrentWindow->ID) {
	ImGuiInputTextState* state = &g.InputTextState;
	ImGui::InputTextDeactivateHook(state->ID);
	state->ID = g.CurrentWindow->ID;
        state->TextA.resize(1);
        state->TextA[0] = 0;
	state->TextLen = 0;
}
				}
...
void TextEditor::SetTextLines(const std::vector<char *> & aLines)
{
	mLines.clear();
...
void TextEditor::Undo(int aSteps)
{
	...
	if (mUndoIndex == 0) mTextChanged = false;
}
...
//		0xff7f7f7f,	// Default
		...
		0xff40ffff, // Identifier
		0xffffffff, // Known identifier
		0xffc040a0, // Preproc identifier
		0x80ffffff, // Comment (single line)
		0x80ffffff, // Comment (multi line)
//		0xff101010, // Background
		0x00000000, // Background
		0xffffffff, // Cursor
		0xe0a06020, // Selection
		0x400020ff, // ErrorMarker
		0x40f08000, // Breakpoint
//		0xff707000, // Line number
		0x4dffffff, // Line number
...
static bool TokenizeNGCParameter(const char * in_begin, const char * in_end, const char *& out_begin, const char *& out_end)
{
	const char *p = in_begin;
...
static bool TokenizeNGCBlockNo(const char * in_begin, const char * in_end, const char *& out_begin, const char *& out_end)
{
	const char *p = in_begin;
...
static bool TokenizeNGCG(const char * in_begin, const char * in_end, const char *& out_begin, const char *& out_end)
{
	const char *p = in_begin;
...
static bool TokenizeNGCNumber(const char * in_begin, const char * in_end, const char *& out_begin, const char *& out_end)
{
	const char *p = in_begin;

	while (p < in_end && ((*p >= '0' && *p <= '9') || *p == '.' || *p == '-' || *p == '+')) ++p;
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::NGC()
{
	static bool inited = false;
	static LanguageDefinition langDef;
	if (!inited)
	{
		...
		static const char* const identifiers[] = {
			"abs", "acos", "asin", "atan", "cos", "exists", "exp", "fix", "fup", "ln", "round", "sin", "sqrt", "tan",
		};
...
//			else if (TokenizeCStyleString(in_begin, in_end, out_begin, out_end))
//				paletteIndex = PaletteIndex::String;
			else if (TokenizeNGCG(in_begin, in_end, out_begin, out_end))
				paletteIndex = PaletteIndex::Default;
			else if (TokenizeNGCBlockNo(in_begin, in_end, out_begin, out_end))
				paletteIndex = PaletteIndex::LineNumber;
			else if (TokenizeNGCParameter(in_begin, in_end, out_begin, out_end))
				paletteIndex = PaletteIndex::Identifier;
			else if (TokenizeNGCNumber(in_begin, in_end, out_begin, out_end))
				paletteIndex = PaletteIndex::Number;
			else if (TokenizeCStylePunctuation(in_begin, in_end, out_begin, out_end))
				paletteIndex = PaletteIndex::Punctuation;
