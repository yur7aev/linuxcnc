...
#define EX_1BSTEP	0x0400	//                  12=EN0 13=D0 14=S0      16=EN1 16=D1 17=S1
...
typedef struct nyx2_dp_fb {
	...
	// 26 dwords free
} _P nyx2_dp_fb;	// 192 dwords max


typedef struct nyx2_dp_cmd {
	uint32_t seq;
...
typedef struct nyx3_dp_cmd {
	uint32_t seq;
	uint32_t gpo[NUM_GPO/32+1];
	uint16_t dac[NUM_DAC];
	uint32_t yo[NUM_YIO*2];
	nyx_servo_cmd servo_cmd[MAX_AXES];
} _P nyx3_dp_cmd;
